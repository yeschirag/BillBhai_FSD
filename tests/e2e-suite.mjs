...
async function runTests() {
  ...
  try {
    ...
    record('Hero Headline Rendered', heroTitle.includes('Run your entire shop'), heroTitle.trim().replace(/\s+/g, ' '))
    ...
    record('Dashboard Charts Rendered', chartExists >= 1)
    ...
    if (productCount > 0) {
      ...
      // Test Promo code WELCOME10
      const promoInput = page.locator('.promo-input input')
      if (await promoInput.count() > 0) {
        await promoInput.fill('WELCOME10')
        await page.click('.promo-input button')
    ...
    record('Reports Analytics & Charts Loaded', true)
